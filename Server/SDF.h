...
*---------------------------------------------------------------------
* 2020/04/17 | 1.0.0      | WinH           | Create file
*---------------------------------------------------------------------
*/
#pragma once
#ifdef __cplusplus
...
#define RSAref_MAX_BITS		4096//2048
...
#define ECCref_MAX_LEN		((ECCref_MAX_BITS + 7) / 8)
	...
	* @param	pucPassword		[IN]	使用私钥权限的识别码，（A-F or 0-9）
	* @param	uiPwdLength		[IN]	私钥访问控制码长度，不少于8字节
	* @return
	*   @retval	0		成功
	*   @retval	非0		失败，返回错误代码
	...
	* @param	hSessionHandle	[IN]	与设备建立的会话句柄
	* @param	uiKeyIndex		[IN]	密码设备存储私钥的索引值
	*									TASS补充：正数获取ECC密钥权限，负数获取RSA密钥权限
	* @return
	*   @retval	0		成功
	*   @retval	非0		失败，返回错误代码
	*/
	...
	* @param	uiKeyBits				[IN]	要求协商的密钥长度，支持128bits（16字节）/192bits（32字节）/256bits（32字节）
	...
	*   @retval	0		成功
	*   @retval	非0		失败，返回错误代码
	* @note	此函数并未包含在GM/T 0018中，但从功能完整性考虑，定义为扩展实现
	*/
	int SDF_ImportKey(
		void* hSessionHandle,
		unsigned char* pucKey,
	...
	* @param	hSessionHandle	[IN]	与设备建立的会话句柄
	* @param	uiAlgID			[IN]	算法标识，指定使用的 ECC 算法
	...
	*   @retval	0		成功
	*   @retval	非0		失败，返回错误代码
	* @note	输入数据为待签数据的杂凑值。当使用SM2算法时，该输入数据位待签数据经过SM2签名预处理的结果，预处理过程见 GM/T AAAA。
	*		此函数并未包含在GM/T 0018中，但从功能完整性考虑，定义为扩展实现
	*/
	...
	* @param	uiFileSize		[IN]	文件所占存储空间的长度，最大8192字节
	* @return
	*   @retval	0		成功
	*   @retval	非0		失败，返回错误代码
	*/
	...
	* @param	puiFileLength	[IN/OUT]	入参时指定读取文件内容的长度；出参时返回实际读取文件内容的长度，最大4096字节
	...
	* @param	uiFileLength	[IN]	指定写入文件内容的长度，最大4096字节
